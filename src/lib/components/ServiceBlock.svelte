...
<div class="w-full mt-6 flex sm:flex-row flex-col items-center">
  ...
  <div class="w-full max-w-lg ml-4">
    <button on:click={toggleHide} class="font-rale text-2xl font-semibold text-graphite-dark hover:text-gray-800 transition-colors" class:text-gray-800={!hidden}>{title}</button>
    {#if !hidden}
